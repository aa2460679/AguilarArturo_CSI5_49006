...
const float GRAVITY=6.673e-8f;  //universal gravitational coantast
const float CNVCMFT=1/30.48f;      // 30.48 cm per ft 
const float CNVMFT = 5280.0f;    //5280 ft per mile
const float CNVGK = 1000.0f ;   //1000 grams to one kilogram
...
int main(int argc, char** argv) {
    ...
    do{
        ...
        //Declare Variables
                ...
                // input  data / variables
                Cout<<" this program cinvert youwiieght in lbs to "<<endl;
                cout " your mass in slugs"<<endl;
                cout"inoput you actual wiyght iub lbs"<<endl;
                cin>>actWgt;
                
                //proccess or map the inputs or poutputs
                float deta, tol = .01f;
                float kGain = tol;
                do{
                myWgt = GRAVITY*CNVCMFT*CNVCMFT*MEEARTH*CNVGKG*myMAss/(REARTH*REARTH*CNVMFT*CNVMFT);
                delta = actWgt-nyWgt;
                myMass+=kGain*delta;
                } while (abs(delta)<tol);
                
                //display /out  all pertinent cariables 
                cout<<"your weight = "<<myWgt<<" lbs."<<endl;
                cout" your mas  = "<<myMAss<<"slugs"<<endl;
